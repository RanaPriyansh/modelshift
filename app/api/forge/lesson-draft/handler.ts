...
import {
  containsAdversarialText,
  containsPrivateData,
  isRestrictedTopic,
} from "../../../../src/lib/forge-planner/safety";
...
const ERROR_MESSAGES = {
  ...
  private_data:
    "Remove personal or private data from this authoring material before trying again.",
  authoring_unavailable:
    "Studio provider requests are unavailable until active adult server-owned authority and required controls are approved.",
  missing_key: "Add a provider API key for this request.",
  request_budget_exceeded:
    "This request exceeds FORGE's fixed time, output, or estimated-cost budget.",
  ...
  provider_error:
    "The provider could not complete the draft. The key was discarded; try again later.",
} as const;
...
async function readBoundedBody(request: Request): Promise<string | null> {
  ...
  if (
    ...
    declaredLength > MAX_LESSON_STUDIO_REQUEST_BYTES
  ) {
    return null;
  }
 ...
 */
export function createLessonDraftPost(
  dependencies: LessonDraftRouteDependencies = DEFAULT_DEPENDENCIES,
) {
  return async function POST(request: Request) {
    ...
    const forwardedAuthoringFields = [
      input.question,
      input.startingPoint,
      input.successShape,
      input.sourceContext,
    ...
    if (forwardedAuthoringFields.some(isRestrictedTopic)) {
      return errorResponse("unsafe_topic", 403, correlationId);
    }
    if (forwardedAuthoringFields.some(containsAdversarialText)) {
      return errorResponse("adversarial_input", 403, correlationId);
    }
    if (forwardedAuthoringFields.some(containsPrivateData)) {
      return errorResponse("private_data", 403, correlationId);
    }
