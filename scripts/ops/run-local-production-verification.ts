...
async function main() {
  ...
  const child = spawn(server.command, server.args, { cwd: process.cwd(), env: serverEnvironment(expectedSha.toLowerCase()), stdio: ["ignore", "pipe", "pipe"] }); const recentLogs = boundedLogs(child);
