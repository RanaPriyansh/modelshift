...
function classifyTest(
  ...
  if (results.length === 0) throw new ReceiptInputError("malformed");
  ...
  if (expectedStatus === "skipped" && (status !== "skipped" || finalStatus !== "skipped")) {
    throw new ReceiptInputError("malformed");
  }
...
export function parsePlaywrightJsonReport(value: unknown): PlaywrightJsonReportSummary {
  ...
  function visitSuite(valueToVisit: unknown, parentFile?: string): void {
    ...
    const suiteFile = suite.file === undefined
      ? parentFile
      : safeRepositoryPath(requiredString(suite, "file", "suite"));
    ...
    suiteSpecs.forEach((valueToVisit, index) => {
      ...
      const specFile = spec.file === undefined
        ? suiteFile
        : safeRepositoryPath(requiredString(spec, "file", `suite.specs[${index}]`));
...
async function writeExclusiveReceipt(
  ...
  const helperSource = writerOptions.helperSource ?? await readTrustedHelperSource(
    resolve(root, hooks.helperPath ?? RECEIPT_HELPER),
    hooks,
  );
  ...
  try {
    ...
    const helperResult = await runExclusiveReceiptHelper(
      root,
      output,
      directory,
      parentStat,
...
async function writeWriterErrorReceipt(options: {
  ...
  const helperSource = await readTrustedHelperSource(RECEIPT_HELPER, {});
...
export async function writePlaywrightEvidenceReceipt(options: {
  ...
  requestedStatus?: PlaywrightEvidenceStatus;
  hooks?: PlaywrightEvidenceTestHooks;
}): Promise<{ receipt: PlaywrightEvidenceReceipt; outputPath: string }> {
  try {
    ...
    try {
      summary = await readTrustedReport(prepared.root, options.target, options.hooks);
    } catch (error: unknown) {
    ...
    return { receipt, outputPath: prepared.output };
  } catch (error: unknown) {
    try {
