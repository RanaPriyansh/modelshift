#!/usr/bin/env python3
"""Commit one bounded output file relative to an inherited trusted directory fd."""
...
def parse_cleanup_arguments(arguments: List[str]) -> Tuple[int, int, int, str]:
    if len(arguments) != 8:
    ...
    for index in range(0, len(arguments), 2):
        key = arguments[index]
        value = arguments[index + 1]
        if key not in {"--dir-fd", "--expected-dev", "--expected-ino", "--prefix"}:
            raise ValueError("invalid cleanup argument")
        if key in values:
            raise ValueError("duplicate cleanup argument")
        values[key] = value
    if set(values) != {"--dir-fd", "--expected-dev", "--expected-ino", "--prefix"}:
        raise ValueError("incomplete cleanup arguments")
    if values["--dir-fd"] != "3" or values["--prefix"] != TEMP_PREFIX:
        raise ValueError("invalid cleanup directory or prefix")
    try:
        expected_dev = int(values["--expected-dev"], 10)
        expected_ino = int(values["--expected-ino"], 10)
    except ValueError as error:
        raise ValueError("invalid cleanup descriptor") from error
    if expected_dev < 0 or expected_ino < 0:
        raise ValueError("invalid cleanup descriptor")
    return 3, expected_dev, expected_ino, values["--prefix"]
...
def read_exact(stream, count: int) -> bytes:
    ...
    while remaining > 0:
        ...
        if not chunk:
            raise ValueError("input ended before its declared size")
        chunks.append(chunk)
        remaining -= len(chunk)
    if stream.read(1):
        raise ValueError("input exceeded its declared size")
...
def read_descriptor_exact(receipt_fd: int, count: int) -> bytes:
    ...
    while remaining > 0:
        ...
        if not chunk:
            raise OSError("output descriptor ended before its declared size")
        chunks.append(chunk)
        remaining -= len(chunk)
    if os.read(receipt_fd, 1):
        raise OSError("output descriptor exceeded its declared size")
...
def create_temporary_receipt(directory_fd: int) -> Tuple[str, int]:
    ...
    for _ in range(MAX_TEMP_ATTEMPTS):
        ...
        try:
            ...
            continue
    raise OSError("could not create a unique temporary output")


def valid_temporary_basename(basename: str, prefix: str) -> bool:
    suffix = basename[len(prefix):]
    return (
        basename.startswith(prefix)
        and len(suffix) == TEMP_TOKEN_LENGTH
...
def cleanup_temporary_receipts(
    ...
    for basename in names:
        ...
        if not is_regular_file(entry_before) or entry_before.st_nlink != 1:
            raise OSError("temporary cleanup entry was not a regular file")
        os.unlink(basename, dir_fd=directory_fd)
        try:
            relative_entry_stat(basename, directory_fd)
        except FileNotFoundError:
            removed += 1
        else:
            raise OSError("temporary cleanup entry remained after unlink")
    directory_after = os.fstat(directory_fd)
    if not is_directory(directory_after) or identity(directory_after) != (expected_dev, expected_ino):
        raise OSError("trusted directory identity changed after temporary cleanup")
    return removed


def cleanup_main(arguments: List[str]) -> int:
    directory_fd, expected_dev, expected_ino, prefix = parse_cleanup_arguments(arguments)
    require_descriptor_support()
    removed = cleanup_temporary_receipts(directory_fd, expected_dev, expected_ino, prefix)
    output = json.dumps({"removed": removed}, separators=(",", ":")).encode("ascii") + b"\n"
    if len(output) > MAX_STDOUT_BYTES:
        raise OSError("cleanup output exceeded its bound")
...
def main() -> int:
    if len(sys.argv) > 1 and sys.argv[1] == "--cleanup":
        try:
            ...
            return fail(f"temporary output cleanup failed: {error}")
    ...
    try:
        ...
        if hashlib.sha256(payload).hexdigest() != expected_digest:
            raise ValueError("input digest did not match its declaration")
        require_descriptor_support()

        directory_before = os.fstat(directory_fd)
        if not is_directory(directory_before) or identity(directory_before) != (expected_dev, expected_ino):
            raise OSError("trusted directory identity changed before output open")
        ...
        if (
            ...
            raise OSError("temporary output descriptor did not match before write")
        ...
        if (
            ...
            raise OSError("temporary output changed after fsync")
        ...
        if (
            ...
            raise OSError("output final entry did not match the temporary descriptor")
        ...
        if (
            ...
            raise OSError("output final entry changed after temporary cleanup")
        ...
        return fail(f"exclusive output helper failed: {error}")
    finally:
