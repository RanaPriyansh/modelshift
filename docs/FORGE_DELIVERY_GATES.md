...
**Current gate posture: DG1 candidate — partial interactive foundation; a bounded production engineering deployment is recorded, but no DG1 PASS packet or terminal production-release decision exists.**
...
- the practical-path strategy, ADR, adult pilot, educator mode, and future scenarios remain C0/request-only, but local code cut `a0eb0758fa1ac029d0ee93c4ded3129fb802f368` now includes separately bounded fixture/pure-module compilers for capability maps, governed resource candidates with a disabled YouTube metadata adapter, representation packages, and practical project/practice packets, plus a contestable unknown-question preview;
- on that exact local code cut, 711/711 application tests, 13/13 evaluator-contract tests, lint, typecheck, diff checks, the optimized build, a 36-static-asset public-boundary scan, a 72 pass / 22 intentional skip / 0 fail browser matrix, and fresh/legacy-upgrade/separate-session-nonce disposable database checks passed;
- those later slices expose useful seams and fail-closed states but are not connected into a reviewed learner route: they add no trusted curriculum/reviewer identity, persistence, publication, assignment, live provider/network behavior, durable proof, adult-pilot entitlement, minor authority, efficacy, or homeschool evidence, and the local code cut is not the recorded public deployment.
- local code cut `39d86820e46fea6e7df4f4dadc48a19a5fa7e7eb` adds `/paths/source-corroboration`, a server-only exact-input presentation projection: it accepts only the known runtime digest `sha256:a172f067f6135bdcec13c66053ef250ef92692db734b60ddf8e396fb8b0dc4b5`, existing released AI & Learning World, exact written-explanation project and non-physics foundation-rehearsal practice templates, and a disabled-provider `no-iframe-request` plan;
- the new route has an ephemeral refresh-clear scratchpad and internal CTA only to the existing AI & Learning source-corroboration World. It performs no iframe/external request/`localStorage`/evidence write, and it leaves reviewed resources, real assignment, durable evidence, adult entitlement, provider, assignment, proof, return, minor, and homeschool authority unavailable;
- on feature code `39d8682`, 721/721 application tests across 75 files, 13/13 evaluator-contract tests across 2 files, lint, typecheck, diff checks, optimized build plus 37-static-asset public-boundary scan, focused path 10/10, targeted desktop/mobile E2E 2/2, and full browser 74 pass / 22 intentional project-scoped skips / 0 fail across 96 cases in 2.3m passed;
- exact source `35a1d2c5bd28c7b794b26414c6bdfe8f18097027` is public at `https://modelshift.vercel.app` through READY production deployment `dpl_Er7rVecXt3iga56P4uPDoLnWt9V4` and immutable URL `https://forge-learning-7a63ywsp5-ranapriyanshs-projects.vercel.app`. Vercel metadata and public `/api/health` identify the same SHA, deployment, project, build time, source digests, and disabled cloud/provider posture;
- exact-main verification passed 790 application tests, 13 evaluator-contract tests, 54 offline fixture-validity checks, typecheck, lint, diff checks, optimized build, and the full local browser matrix at 74 pass / 22 intentional project-scoped skips / 0 fail. Live Chrome smoke confirmed keyboard route-entry focus, no desktop/320px horizontal overflow, 49–63px radio-label targets, and zero console warnings/errors;
- the public verifier passed 206 checks and failed four release-identity checks. The deployment manifest asserted a locally precomputed public-asset digest `b1f8d95f...`, while Vercel emitted `aad493295...`; a same-source non-promoted experiment emitted a third digest `83e1de1a3...`. The pre-build digest is not a valid receipt for the deployment-specific remote output;
- the public app therefore establishes bounded exact-code operation, but its release state is `DEPLOYMENT_BLOCKED`, not `DEPLOYED_CANDIDATE` or `PRODUCTION_VERIFIED`. A post-build provider receipt, a later passing candidate, rollback rehearsal, live provider/database operation, manual assistive-technology coverage, and every learning-validity gate remain absent.

The strongest currently permitted claim is:

> **FORGE is a C1 interactive foundation and DG1 candidate. Four authored learning Worlds demonstrate bounded planner-to-World, proof-after-help, browser-local evidence, and an unverified AI lesson-draft compiler. The current exact-source app is public and its learner surface passes the named engineering checks, but the release is `DEPLOYMENT_BLOCKED` by a contradictory deployment-asset receipt. A public adult presentation route makes a fixture-only map-to-World/resource/project/practice sequence inspectable, while external-resource, assignment, proof, and return actions remain unavailable. The broader reviewed curriculum, trust core, homeschool use, educational effects, child-safety operation, live provider behavior, durable identity/persistence, and terminal production readiness remain unproven.**
