...
`release_manifest` is `unbound` for local builds and for any incomplete/malformed public tuple. It becomes `bound` only for an explicit `DEPLOYED_CANDIDATE` with one exact source SHA, canonical build time, lock digest, Vercel-owned immutable deployment ID/URL/project identity, the checked-in alias URL, and `public_asset: provider_receipt_required`. `VERCEL_GIT_COMMIT_SHA` must equal the configured source SHA; `VERCEL_DEPLOYMENT_ID`, `VERCEL_URL`, and `VERCEL_PROJECT_ID` must be present and satisfy the checked-in FORGE project/hostname policy. Immutable and alias URLs must be canonical HTTPS origins: no credentials, IP/localhost host, non-default port, path, query, or fragment; explicit `:443` normalizes away. Caller `FORGE_RELEASE_DEPLOYMENT_ID` and `FORGE_RELEASE_IMMUTABLE_URL` are checked only for contradiction and cannot override platform identity. Caller alias URL or alias-resolution timestamp values are rejected: post-deploy alias time cannot be truthfully embedded in the deployment. A build-supplied `FORGE_PUBLIC_ASSET_DIGEST`, including the former absence gate, is rejected: Vercel has not emitted the deployment-specific tree when health is built. The metadata authority for a remote candidate is the bound health manifest, checked-in target policy, expected source/digests, the verifier's alias receipt, and an in-process receipt collected through authenticated Vercel deployment metadata plus build logs. The verifier requires `build_time <= provider-observed asset-log time` and `build_time <= alias_verified_at`; it rejects unbound, malformed, contradictory, source-drifted, project/alias-mismatched, stale, wrong-deployment, or self-reported-only receipts. A saved JSON receipt is explicitly external evidence and remains blocked; it does not become provider proof merely by naming a deployment.
