...
| retained ModelShift regression | `pnpm eval` and sanitized evaluation report; this is not Semester Desk or efficacy evidence | `FAIL` |
...
`release_manifest` schema `2.0` is `unbound` for local builds and for any incomplete or malformed public tuple. It becomes `bound` only for an explicit `DEPLOYED_CANDIDATE` with one exact provider Git SHA, lock digest, Vercel-owned immutable deployment ID/URL/project identity, the checked-in alias URL, and `artifact: provider_receipt_required`. Schema `1.0` and its `public_asset` field are retired and fail closed. `VERCEL_GIT_COMMIT_SHA` is the sole manifest source SHA; caller `FORGE_RELEASE_SHA` cannot override it. `VERCEL_DEPLOYMENT_ID`, `VERCEL_URL`, and `VERCEL_PROJECT_ID` must be present and satisfy the checked-in FORGE project/hostname policy. Caller deployment/immutable values are contradiction checks only, and caller alias or alias-resolution values are rejected. A build-supplied `FORGE_PUBLIC_ASSET_DIGEST`, including the retired absence gate, is rejected because Vercel has not emitted the deployment-specific tree when health is built.

Remote candidate authority comes from a same-process opaque capability issued only after fixed-origin authenticated Vercel reads. The collector requires a `production` and `READY` deployment, the checked-in project and immutable-host policy, provider-owned `gitSource.type/repoId/ref/sha`, and provider-owned `gitRepo.namespace/name/path/type/defaultBranch`. The GitHub repository ID is `1308085427`; caller-settable deployment `meta` is consistency-only and never supplies source or repository identity. Provider responses stream under a hard four-megabyte actual-byte cap with TLS certificate verification enabled. The build log must contain exactly one canonical `FORGE_PUBLIC_BUILD_ARTIFACT_MARKER.v2` line. The marker binds the complete production receipt, `runtimeCachePolicy`, and public-asset file count. Receipt schema `2.0` requires a clean source state, an exact Git tree, the source-bound build ID, all digests, and all file counts. The verifier compares the Git tree, public directory, and runtime configuration with the checked-out source. Provider deployment creation must precede the marker, and the marker must precede the verifier's alias fetch. Retired markers, receipt schema `1.0`, plain JSON, a hand-written authority label, preview/CLI-local source, project/source drift, and a self-reported digest remain `DEPLOYMENT_BLOCKED`.
...
The read-only deployment workflow runs only from `main`. Its `expected_sha` input must equal the exact checked-out `GITHUB_SHA`. The verifier also requires the health payload and response header to report that compiled build source SHA.
...
The verifier accepts a checked-in `--target-id` for remote inspection (or explicit `--allow-localhost` for a local artifact only), a full 40-character expected SHA, and—only for a remote candidate—an exact production Vercel deployment ID plus the name of an environment variable holding an API token. It collects bounded deployment metadata and build events from `api.vercel.com`, issues an opaque process-local receipt capability, and then performs bounded same-origin `GET` requests with redirects disabled. It verifies `/api/health`, `/`, `/app`, `/how-forge-works`, `/university`, `/privacy`, `/terms`, `/support`, and same-origin versioned Next.js scripts. It also requires representative retired paths to return `404`. It checks exact SHA, the checked-in alias, immutable/project identity, the authenticated provider tuple, source/repository policy, provider event ordering, security headers, route markers, and client secret-pattern absence. It never submits forms or calls write paths, and it does not claim byte-for-byte proof of every remote edge asset.
...
The script scan is a hard initial-HTML contract: it unions the distinct allowed `/_next/static/` script URLs referenced by the canonical route set, requires a nonempty set of at most 32, then fetches and scans every admitted asset. The public-asset digest and count cover the complete emitted `.next/static` tree. The complete artifact digest covers the bounded production `.next` tree. The read-only network scan remains limited to initial HTML assets. Hydration-only chunks are outside the network scan. Increasing the budget requires a new observed-count measurement, boundary-test update, and release-operations review.

The canonical Semester Desk browser gate uses four journeys: public desktop, student desktop, public mobile, and student mobile. The public journeys inspect all final public pages and the `320` CSS pixel stress case. The student journeys run the complete device-local Semester Desk flow, including recovery, study, proof, delayed return, export, and reset. Each journey checks the final route, landmark structure, overflow, focus, and browser errors. Historical 53-page ModelShift captures are evidence for the retired product only.
