...
- platform-owned exact `VERCEL_GIT_COMMIT_SHA`; `FORGE_RELEASE_SHA` is ignored for a bound candidate and cannot override it
- `FORGE_BUILD_TIME`, if present, is health-only diagnostic metadata and is not release provenance
...
Any malformed field, caller-supplied asset assertion, missing platform field, or caller alias receipt produces `unbound`. Expected-source drift is blocked by the remote verifier, which compares its expected SHA with both health and the receipt. A remote `DEPLOYED_CANDIDATE` additionally requires a process-local capability issued in the same verifier run by the fixed-origin authenticated Vercel collector. It requires provider-owned `gitSource.type`, `gitSource.repoId` equal to checked-in GitHub repository ID `1308085427`, `gitSource.ref`, and `gitSource.sha`, plus provider-owned `gitRepo.namespace`, `gitRepo.name`, `gitRepo.path`, `gitRepo.type`, and `gitRepo.defaultBranch` matching the checked-in repository policy. Caller-settable `meta` is consistency-only and never supplies source or ownership. It also requires exact deployment ID/project/immutable URL, READY state, provider `createdAt`, and the provider timestamp/digest parsed from `Public build boundary verified across … public asset digest <sha256>.` The verifier requires provider creation time no later than that terminal log event and that event no later than its independently fetched `alias_verified_at`. A saved JSON receipt or a hand-written authority object is external evidence and is blocked; neither can recreate the process-local capability. This is authenticated transport evidence, not a cryptographic signature or byte-for-byte proof of every remote edge asset.
