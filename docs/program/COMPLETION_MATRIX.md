...
**Current local main:** `de84fcbc6dab0b5774f658e7efc2b635fcfef169` is not deployed. It adds independently accepted immutable lock identity, exact all-World ADR-001 receipt compilation, and a pure source-authority contract/replay boundary. Its integrated local gate passed 82 focused tests, 574 application tests, 13 evaluator-contract tests, typecheck, lint, exact retained release digests, and diff checks. SQL durability, curriculum graph coverage, the fifth World, browser/access integration, exact-SHA CI, and any later deployment remain incomplete.
...
| PATH-01 | Homeschool/self-study pathways preserve breadth, choice, access, relationships, protection, and portability | Packet C availability surface plus principal curriculum-graph specification `8cf558c` | `IN_PROGRESS`; graph candidate not yet accepted | The public map still names four released capabilities and explicit gaps only. The active pure graph slice must separate release from legacy-incomplete source authority and expose all nine areas without selecting a learner path. No schedule, recommendation, learner choice operation, jurisdiction pack, attendance, accreditation, coverage sufficiency, or homeschool readiness exists. | Independently accept curriculum graph; later learner-choice and jurisdiction gates remain separate |
...
| G1 Trust core and durable replay | `PARTIAL`; exact all-World v2 compiler and in-memory replay accepted on local main | No accepted SQL parity/durability, authenticated/server proof authority, application append, configured adult-only operation, concurrency/backup/restore/rollback production proof. |
| G2 Learning Kernel and World factory | `PARTIAL`; four-World shared runtime plus exact compiler accepted | W5-C SQL and W5-D curriculum graph remain active; every future World still requires domain/runtime/source/content/access review before registry release. |
