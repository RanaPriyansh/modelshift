...
**Current accepted local code baseline:** `c708f18b795ee105139d4c049cb01e2a68005a52` is not deployed; this later matrix amendment is documentation-only. The code baseline adds independently accepted immutable lock identity, exact all-World ADR-001 receipt compilation, pure source-authority replay, a pure nine-area curriculum-graph contract, and the additive local SQL persistence contract. Its exact-SHA gate passed 14 focused SQL/compiler tests, 593 application tests, 13 evaluator-contract tests, typecheck, lint, and diff checks. The disposable PostgreSQL gate passed fresh and historical-upgrade fixtures plus 12 overlapping cross-version races, deleted both named databases, and left zero `forge_w5c_*` fixtures. The graph still cannot emit a routable route, and local SQL evidence is not hosted/production durability. A trusted registry/source adapter, application persistence, the fifth World, browser/access integration, exact-SHA CI, and any later deployment remain incomplete.
...
| RUNTIME-01 | One modular World authoring/runtime contract | `src/forge/contracts.ts`; `src/forge/world-runtime/**`; four adapters; receipt-only compatibility projector; Wave 4 `c7d4011` | `PARTIAL`; four-World runtime accepted code | All four current Worlds have runtime bindings and passed 18/18 all-World conformance plus terminal routed browser 8/8. Receipts/projector output remain local, non-durable, unauthenticated, and not trusted pathway evidence. | Durable evidence architecture after additive v2 SQL/golden-fixture review |
| RUNTIME-02 | Domain truth remains domain-specific | Four World reducers/validators; runtime adapter projection; canonical validator registry | `PROVEN` for four bounded authored slices | Each domain owns reducer/validator truth while the shared runtime owns sequencing/policy; all-World reducer equivalence and bounded pass/fail/malformed behavior passed. Construct validity beyond the authored tasks remains untested. | Research validity and future World review |
| PROOF-01 | Help visibly leaves before unfamiliar proof | Four World reducers/components; shared runtime proof locks; terminal routed browser 8/8 | `PARTIAL`; four runtime-enforced bounded flows | All four runtime Worlds reject instructional/model/replay actions in proof while retaining typed access; the terminal matrix rendered bounded results and one receipt-derived local entry per route. Complete nonvisual equivalence and manual AT proof remain absent. | Experience/manual AT audit |
...
| EVID-01 | Learner-owned bounded evidence, export, and deletion | Device ledger plus four bounded local runtime receipts and terminal browser tests | `PROVEN` for privacy-minimal device records; `PARTIAL` overall | All four runtime receipts name task/content/validator/support/access/source status and exclude raw learner prose, but remain client-controlled, non-durable, and source-incomplete. Device records remain mutable local storage; no durable authenticated application authority exists. | Trust projector/persistence after additive review |
| EVID-02 | Every World emits one canonical evidence vocabulary | All four bounded runtime receipts compile through the accepted W5-B chain ending at local main `de84fcb` | `PARTIAL`; exact all-World compiler accepted, not application or durable authority | The compiler re-runs each canonical validator and pins receipt, retained manifest, fresh package/runtime digests, exact task code, support/access/source facts, package-owned limitations, event identities, and coherent in-memory replay. It emits no raw learner prose. No route invokes durable append, no authenticated/server proof nonce exists, v1 device writers remain, and SQL parity is still in progress. | Accept additive v2 SQL/golden parity before any persistence binding |
...
| G2 Learning Kernel and World factory | `PARTIAL`; four-World shared runtime, exact compiler, pure curriculum graph, and local SQL parity accepted | The fifth retained package remains rejected pending domain/content/public-boundary repair; every future World still requires independent runtime/source/curriculum/content/access acceptance before public release. |
