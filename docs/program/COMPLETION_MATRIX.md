...
| ACCESS-01 | One coherent accessible learner instrument | Integrated shared tokens/primitives; Packet A fidelity report; production-mode cross-route browser matrix on `f7a3bc4` | `PARTIAL`; Packet A `ACCEPTED` | Home, Studio, login/account, all four Worlds, evidence, and trail now have explicit 320 px target/input/overflow checks plus rendered contrast, keyboard/skip-link, reduced-motion, forced-colors, and increased-contrast evidence. Screen-reader sessions, complete graph/vector/table/text alternatives, and all-state nonvisual equivalence remain incomplete. | Packet E hooks + later assistive-technology audit |
| SAFE-01 | Security/privacy/abuse boundaries support enabled cloud/provider use | CSP nonce, SSR cookie tests, origin/body/schema limits, fixed providers, structurally disabled Packet B cloud boundary | `PARTIAL`; cloud enablement remains forbidden | The current code fails closed rather than pretending an in-process limiter or adult checkbox is operational abuse control. No CAPTCHA/durable limiter, configured-project/provider proof, prompt/source poisoning operating test, provider privacy decision, deletion/backup drill, incident operator, or verified public runtime exists. | Trust B follow-up, Lesson F, Release D |
| EVAL-01 | Engineering behavior is freshly verified | Integrated SHA `9a4068f`: lint, typecheck, 249 application tests, 9 evaluator tests, optimized build, fresh/legacy-upgrade SQL fixtures, and 55-pass / 19-intentional-skip / 0-fail production Chromium matrix | `PARTIAL` | Current A/B/C code is green with local database upgrade proof and full rendered Chromium coverage. Packet D/E/F integration, live configured database/provider checks, Safari/Firefox, screen-reader sessions, and the release-identity/public-deployment matrix remain unrun. | Release D + owning lanes |
...
| G4 Foundations and learner frontier | `PARTIAL`; pure pathway contract accepted | Packet E trusted-receipt integration, reviewed curriculum packages, prerequisite/capability graphs, and actual broad/deep learner paths. |
