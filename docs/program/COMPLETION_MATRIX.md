...
| G1 Trust core and durable replay | `PARTIAL`; exact all-World v2 compiler, in-memory replay, and local additive SQL contract accepted | No authenticated/server proof authority, application append, configured adult-only hosted operation, backup/restore, recovery, rollback, or production durability proof. |
| G2 Learning Kernel and World factory | `PARTIAL`; four-World public runtime, retained internal W5-E package, exact compilers, pure curriculum graph, and local SQL parity accepted | W5-E and every future World still require independent trusted-source, curriculum, content, routed browser/access, publication, and release acceptance before becoming public. No current receipt is durable/authenticated authority. |
...
| G4 Foundations and learner frontier | `PARTIAL`; availability-only public map plus pure content-addressed nine-area graph accepted | A trusted registry/source adapter, durable reviewed receipts/source service, reviewed curriculum packages, learner choice, and actual broad/deep paths remain absent. |
| G5 Unified learner experience/access | `PARTIAL`; Packet A, typed access on four runtime Worlds, and all-World automated browser conformance accepted | Screen-reader sessions, complete semantic alternatives, all-state nonvisual equivalence, real assistive-technology review, and Safari/Firefox remain unrun. |
