...
**Wave 6 current boundary:** exact integrated and deployed source is `35a1d2c5bd28c7b794b26414c6bdfe8f18097027`. It retains the fixture/pure-module capability-map, resource/YouTube, representation, project/practice, and `/paths/source-corroboration` presentation boundaries; adds the accepted Wave 6 authority preflight, frontend route/accessibility/planner repairs, a deterministic non-routed adult-pilot controller kernel, and release-manifest/verifier foundations. On exact main, 790 application tests, 13 evaluator-contract tests, 54 offline fixture-validity checks, lint, typecheck, diff checks, optimized build, and the full browser matrix at 74 pass / 22 intentional project-scoped skips / 0 fail passed. Source `35a1d2c` is public at `https://modelshift.vercel.app` through READY deployment `dpl_Er7rVecXt3iga56P4uPDoLnWt9V4` and immutable URL `https://forge-learning-7a63ywsp5-ranapriyanshs-projects.vercel.app`; platform metadata and public health bind the exact SHA, deployment/project identity, build time, lock/content/evaluator digests, and disabled cloud/provider posture. Live Chrome keyboard/320px smoke passed without overflow or console warnings/errors. Release verification passed 206 checks and failed four: the manifest asserted local asset digest `b1f8d95f...`, while Vercel emitted `aad493295...`; a same-source non-promoted experiment emitted `83e1de1a3...`. The app is therefore public but `DEPLOYMENT_BLOCKED`, never production verified. The controller is not a route, provider, persistence, entitlement, assignment, or evidence-authority path, and W6-006 remains incomplete. No reviewed resource, real assignment, durable proof, minor operation, or homeschool authority follows.
...
| Release, evaluation, observability | `019f8f92-8e02-7cf3-a417-a0314852eaa6` | lower-cost implementation lane | `IN_PROGRESS`; post-build provider-receipt repair dispatched | integrated provenance foundation `35a1d2c`; public `dpl_Er7rVecXt3iga56P4uPDoLnWt9V4`; verifier 206/210 | Replace invalid pre-build asset expectation with an exact-deployment provider-observed receipt; no promotion until a later candidate passes |
| Adult pilot controller and route shell | `019f8f95-657f-7c53-8e09-aba52437238d` | lower-cost implementation lane | `IN_PROGRESS`; pure kernel `ACCEPTED_ON_MAIN`, fail-closed route shell dispatched | source `3ab4aa9`; integrated as `07fd770`; 16/16 focused tests and final adversarial review | Return a cherry-pickable inaccessible-by-default route shell without inventing entitlement, provider, persistence, assignment, proof, or release authority |
...
Next dependency order: repair the post-build provider receipt and verify a later deployment; build the fail-closed adult route shell around the accepted controller without making it publicly available; connect no fixture contract to trusted runtime authority until source/reviewer and adult server-entitlement authority are accepted; then build one bounded reviewed map-to-resource-to-project route with explicit support withdrawal and return. Educator operation, live provider access, minor paths, durable proof, and homeschool claims remain separate later gates.
