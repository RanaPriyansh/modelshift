...
**Wave 6 current boundary:** code cut `39d86820e46fea6e7df4f4dadc48a19a5fa7e7eb` follows the bounded chain recorded below and is distinct from the recorded public release. Capability-map, resource/YouTube, representation, and project/practice contracts remain fixture/pure-module scope; this cut adds `/paths/source-corroboration`, an adult presentation-only fixture path. Its server-only projection admits only the exact runtime digest `sha256:a172f067f6135bdcec13c66053ef250ef92692db734b60ddf8e396fb8b0dc4b5`, the existing released AI & Learning capability at `/learn/ai-and-learning`, exact written-explanation project and non-physics foundation-rehearsal practice templates, and the disabled `no-iframe-request` provider plan. On this exact code cut, 721/721 application tests across 75 files, 13/13 evaluator-contract tests across 2 files, lint, typecheck, diff checks, optimized build, 37-static-asset public-boundary scan, focused path 10/10, targeted desktop/mobile E2E 2/2, and the full browser matrix 74 pass / 22 intentional project-scoped skips / 0 fail across 96 cases in 2.3m passed. The documentation SHA, CI/release identity, and any later deployment remain separate principal records. The architecture/ADR and adult pilot remain proposed. The presentation route has an ephemeral refresh-clear scratchpad and an internal CTA only; it makes no iframe/external request/`localStorage`/evidence write and grants no reviewed resource, real assignment, durable evidence, adult entitlement, provider, assignment, proof, return, deployment, minor, or homeschool authority.
...
| Release, evaluation, observability | `019f8705-4855-7522-9140-8d45cd93ae60` | `gpt-5.6-luna` high | `IDLE`; Wave 4 exact public candidate `RECORDED` for `c7d4011` | exact-main CI `29944461461`; deployment `dpl_6MUabxtnuMVzwxHf1vbM5Y1LxNfs`; bound verifier 187/187 | Fix the unbound lock-digest reproducibility mismatch and run separately authorized rollback/alerting work; do not upgrade `DEPLOYED_CANDIDATE` to terminal release closure |
...
| Independent real-world principal audit | `019f8e78-550b-7150-82d5-2acb958dab37` | `gpt-5.6-sol` ultra | `WAITING_FOR_FROZEN_SHA`; read-only, one pass | [`PRINCIPAL_PRODUCTION_AUDIT_MANDATE.md`](./PRINCIPAL_PRODUCTION_AUDIT_MANDATE.md) | Run after the current implementation/documentation tranche has an exact integrated SHA; return steelman, P0–P3 red team, and recommendation-only disposition with no implementation/review loop |
...
| 6 | Source-corroboration presentation projection | `39d86820e46fea6e7df4f4dadc48a19a5fa7e7eb` | `ACCEPTED_ON_MAIN` for the bounded local presentation; release remains principal-owned | Server-only exact-input projection exposes `/paths/source-corroboration` against the known runtime/public-release/project/practice/disabled-provider tuple. The sole CTA opens the existing AI & Learning World. The scratchpad is in-memory and refresh-clear; no iframe, external request, `localStorage`, evidence write, provider, assignment, proof, or return action occurs. Focused path 10/10, targeted desktop/mobile E2E 2/2, and full browser 74 pass / 22 intentional project-scoped skips / 0 fail across 96 cases in 2.3m passed. |

Next dependency order: record the exact documentation SHA; connect no contract to runtime until trusted source/reviewer and adult server-entitlement authority are accepted; then build one bounded reviewed map-to-resource-to-project route with explicit support withdrawal and return. Any CI/deployment is a separately bound release record. Educator operation, live provider access, minor paths, durable proof, and homeschool claims remain separate later gates.
