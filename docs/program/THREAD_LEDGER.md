...
**Wave 6 current boundary:** the latest verified local main contains the fixture/pure-module capability-map, resource/YouTube, representation, project/practice, `/paths/source-corroboration` presentation, Wave 6 authority preflight, frontend repairs, deterministic controller, fail-closed `/pilot` review shell, and provider-authenticated post-build receipt. Combined local verification passed 819 application tests, 13 evaluator-contract tests, 54 offline fixture-validity checks, lint, typecheck, optimized 42-asset build with the pilot marker gate, production dependency audit, and the full browser matrix at 76 pass / 28 intentional project-scoped skips / 0 fail. Focused production pilot checks added 4/4 default-denial/storage passes and 3/3 enabled review-fixture passes. The currently recorded public source is still `35a1d2c5bd28c7b794b26414c6bdfe8f18097027` at `https://modelshift.vercel.app`, READY deployment `dpl_Er7rVecXt3iga56P4uPDoLnWt9V4`, and remains `DEPLOYMENT_BLOCKED` under the retired pre-build digest contract. The new local candidate has not yet been deployed or provider-receipt verified. The pilot shell is review infrastructure, not provider, persistence, entitlement, assignment, cohort, or evidence authority, and W6-006 remains incomplete. No reviewed-resource operation, real assignment, durable proof, minor operation, or homeschool authority follows.
...
| Release, evaluation, observability | `019f8f92-8e02-7cf3-a417-a0314852eaa6` | lower-cost implementation lane | `ACCEPTED_ON_MAIN`; provider-authenticated receipt implemented | `a3c2090` → `5783c6e` → `ed03187` → `16e048e`; 101 focused tests and independent red-team GO | Requires a fresh Git-connected production deployment and live read-only receipt collection; legacy CLI deployment remains blocked |
...
| 10 | Principal deployment record | deployed source `35a1d2c5bd28c7b794b26414c6bdfe8f18097027`; `dpl_Er7rVecXt3iga56P4uPDoLnWt9V4` | `DEPLOYMENT_BLOCKED` | READY public target at `https://modelshift.vercel.app`; exact platform/health tuple and live learner smoke pass. Public verifier is 206/210 because manifest asset digest `b1f8d95f...` contradicts provider-emitted `aad493295...`; release identity and state binding fail closed. Rollback, manual AT, live provider/database, and learning-validity gates remain unrecorded. |
| 11 | Provider-authenticated post-build receipt | `a3c2090` → `5783c6e` → `ed03187` → `16e048e` | `ACCEPTED_ON_MAIN`; not yet exercised against a fresh candidate | Only an opaque same-process capability from fixed-origin provider reads can bind a `production`/`READY` Vercel Git deployment with exact repository ID/ref/SHA and emitted asset marker. Preview, CLI/local-source, plain JSON, oversized response, project/repo drift, and app build-time provenance fail closed. |
