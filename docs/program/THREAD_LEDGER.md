...
**Wave 3 release boundary:** released source is `98687eaac20f956698000467cd01ea1da9b7fd6e`, public alias is `https://modelshift.vercel.app`, production deployment is `dpl_pLpQmuCP43Mc3tNt1nErf8MRH2v2`, and immutable URL is `https://forge-learning-a3jw66bda-ranapriyanshs-projects.vercel.app`. The bound verifier status is `DEPLOYED_CANDIDATE`; live evaluation and rollback rehearsal remain `NOT_EVALUATED`. This later documentation-only amendment is not deployed. No lane may describe the release as durable authority, live provider operation, cloud auth, all-World runtime coverage, broad curriculum, certification, or homeschool readiness.
...
| Release, evaluation, observability | `019f8705-4855-7522-9140-8d45cd93ae60` | `gpt-5.6-luna` high | `IDLE`; Wave 3 exact public candidate `RECORDED` for `98687ea` | Packet D source `1db691e`; Wave 3 CI `29932913141`; deployment `dpl_pLpQmuCP43Mc3tNt1nErf8MRH2v2` | Investigate the retained first-request browser timeout and run separately authorized rollback/alerting work; do not upgrade `DEPLOYED_CANDIDATE` to terminal release closure |
| Learning Kernel and World Factory | `019f8973-ef2f-7071-89a3-d5b81781957d` | `gpt-5.6-terra` xhigh | `IDLE`; Primary Source + Ratio runtime and ADR-001 projector `ACCEPTED_ON_MAIN` | Primary runtime `abf34c2`; projector `4dab5d9` + `f4179e9`; Wave 3 hardening/migration through `98687ea` | Migrate Force and Motion and AI & Learning, then establish all-World conformance; local receipts/projector output are neither durable nor trusted pathway evidence |
...
The next dependency order is now Force and Motion runtime migration; AI & Learning runtime migration; all-World runtime conformance; additive v2 SQL/golden-fixture review before persistence; durable source authority; broader reviewed curriculum/path graphs; and manual assistive-technology review. Cloud identity, provider enablement, certification, and homeschool operation remain separate authorization tracks.
