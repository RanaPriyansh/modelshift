...
**Wave 2 code boundary:** accepted `main` code is `b5b3170094ecaac5292c7fd23a8b47d86ccc1a9d`. The identified public production source remains `cd418b8a5bc9784fb5e4add3a2286d011fdbdae0` until a principal release tuple binds a later source SHA to an immutable READY deployment, alias, health identity, retained verification artifact, and rollback target. No Wave 2 lane may describe code acceptance as deployment, durable authority, or live operation.
...
| Release, evaluation, observability | `019f8705-4855-7522-9140-8d45cd93ae60` | `gpt-5.6-luna` high | `IDLE`; Packet D plus Wave 2 `/pathways` verifier coverage `ACCEPTED_ON_MAIN` | Source `1db691e`; exact-base replay/integration `1c5dd3a`; route coverage `b5b3170` | Hold for a principal final release tuple; no deploy, public-release, or live-operation conclusion from verifier coverage alone |
...
| 4 | Release-verifier integration | `b5b3170` | `ACCEPTED_ON_MAIN` code coverage | The verifier requires the pathway marker; it does not deploy, establish a public URL, or prove a production tuple. |
| 5 | Principal documentation reconciliation | `docs/program` amendment on current `b5b3170` | `DOCUMENTATION_ONLY` | Records accepted code and known gaps without upgrading production, authority, provider, curriculum, or child-cloud claims. |
