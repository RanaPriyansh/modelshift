...
**Wave 5 current boundary:** [`WAVE_5_PLAN.md`](./WAVE_5_PLAN.md) remains the active architecture packet. Principal accepted local code baseline `29e1aaeccdb4df6e6893230ec8ee0fbaea9c73d0` includes independently accepted W5-A immutable lock identity, W5-B all-World receipt compilation, the source-authority half of W5-D, and the pure nine-area curriculum-graph contract; the later ledger amendment is documentation-only. On that exact code SHA, 18 focused curriculum tests, 592 application tests, 13 evaluator-contract tests, typecheck, lint, and diff checks passed. It is not deployed. The graph exposes only `caller-asserted-unverified` exact matches with null routable routes; it is not a trusted curriculum/publication adapter. W5-C SQL correction is in independent re-review, and the retained-unavailable W5-E implementation is in corrective review after public-fixture and duplicated-domain-truth findings. No worker may enable cloud/provider features, migrate a live database, publish a World, push shared main, deploy, or edit principal program documents.
...
| 4 | Additive ADR-001 v2 SQL correction | candidate `5e5646e` + outbox repair `4bbdf0d`; original `0689309` remains rejected evidence | `INDEPENDENT_REVIEW_IN_PROGRESS` | Local fresh/upgrade/golden and two-session race evidence is candidate-only until re-review accepts truncate refusal and the upgrade-safe bounded error-code invariant. No production durability follows. |
