...
**Wave 6 current boundary:** code cut `a0eb0758fa1ac029d0ee93c4ded3129fb802f368` adds the bounded chain recorded below and is distinct from the documentation commit that follows it. Capability-map, resource/YouTube, representation, and project/practice contracts were accepted only in fixture/pure-module scope; the learner-facing addition is a contestable unknown-question preview whose revision note is explicitly local and unsaved. On this exact code cut, 711/711 application tests across 73 files and 13/13 evaluator-contract tests across 2 files passed, as did lint, typecheck, diff checks, the optimized build, and the 36-static-asset public-boundary scan. The full browser matrix passed 72 with 22 intentional project-scoped skips and zero failures. The disposable PostgreSQL harness passed fresh, legacy-upgrade, and separate-session nonce-race cases before both `forge_w5c_*` databases were deleted and their absence verified. The documentation SHA, CI/release identity, and any later deployment remain separate principal records. The architecture/ADR and adult pilot remain proposed. No slice is connected into an end-to-end reviewed learner route, and none grants curriculum, reviewer, identity, publication, assignment, network/provider, persistence, proof, deployment, minor, or homeschool authority.
