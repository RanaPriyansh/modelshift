...
envelope with locally computed content identities. Mechanical checks do not
establish candidate render binding, rendered parity, pack equivalence,
real-entity exclusion, approval, or rehearsal readiness. Participant
recruitment, enrollment, observation, and capture remain blocked, and
...
| `UV1-OBS-014` | Replace placeholder substitute and P/Q plan hashes with exact full-content synthetic artifacts. | Strict artifact schemas bind every scenario's term/course, source, deadline, capacity, path, World, terminal state, choices, bounded job, effects, answer key, and difficulty counts; seven locked information IDs explicitly cover all nine named information/effect categories. Frozen exact P/Q manifests and semantic oracle, frozen v1 visible copy, seven explicit neutral ordinal navigation labels, frozen substitute and renderer declarations, globally unique references, bounded density that counts the navigation labels, supplied-versus-expected mismatch identities, and domain-separated local identities cover scenario, pack, candidate-fixture, candidate-adapter, renderer, substitute, moderator, review, and projection manifests. | Implemented as local synthetic artifact candidates; candidate build identity remains caller asserted, while adapter and renderer identities are locally recomputed manifest-only; authenticity, real-entity exclusion, runtime binding, rendering, and approval not established |
| `UV1-OBS-015` | Keep mechanical artifact comparison separate from independent equivalence and participant authority. | Maximum artifact state is `mechanical_parity_passed_review_required`; the readiness comparator labels information/task equality as one shared declared manifest; projection keeps candidate/substitute rendering `not_rendered`, independent review `requested`, all six gates open, and every enrollment/capture/persistence/send/publish/claim authority false. | Implemented for bounded local preflight; independent surface compilation, rendered parity, review, approval, rehearsal, and participant authority open |
