...
| `UV1-OBS-013` | Bind production-mode browser evidence to one exact clean source and copied runtime artifact rather than a server-start release assertion or mutable working build. | The build carries a build-time full-SHA source marker; a strict post-build receipt binds the clean source SHA and tree, build ID, the immutable production `.next` set and file count, emitted static assets, the full `public` directory, and a fresh-ephemeral `.next/cache` policy. Declared development, diagnostic, trace, receipt, and runtime-cache paths are outside the immutable set. The harness validates the clean exact-SHA receipt, copies the measured bytes into an isolated snapshot, seals the immutable paths, leaves only a fresh cache location writable, and repeats receipt, marker, build ID, artifact, and public identities before startup and after shutdown. Any immutable drift fails the run. | Implementation candidate; a clean committed-SHA browser receipt remains to be collected. No hermetic, reproducible, signed-build, trusted-builder, dependency-provenance, deployment, provider, remote-artifact, or public-alias identity follows. |
