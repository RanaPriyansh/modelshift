...
| `UV1-OBS-017` | Compile every canonical P/Q scenario through the existing raw semester-loop inputs instead of constructing a display projection or trusting the authored expected status. | Callers may select only a frozen pack ID and scenario ID. The compiler derives exact course-source reconciliation, Today, Recovery, and World inputs; invokes the current semester-loop projector, which recomputes its child projections; verifies scope, copied source, deadline, declared capacity, learner-owned path, exact World binding, terminal state, and no-effect authority; and uses `expectedStatus` only as a final postcondition. The raw fixture is returned only as a digest and every usable result is deeply frozen. | Implemented with focused tests in the local working candidate; the exact clean-source test receipt, accepted candidate SHA, build digest, and adapter ledger still require the two-commit freeze before runtime binding can be claimed |
| `UV1-OBS-018` | Compile the comparable candidate and neutral content from one exact local surface packet rather than maintaining two visible-copy implementations. | The packet binds the exact authored pack and renderer digests, seven ordinal navigation items, fixed node order, exact facts, choices, bounded next job, local-fragment primary-control effect, no-effect table, nine tasks, terminal limits, copybook, and per-scenario density ceiling. Renderer-specific hierarchy and brand may differ only outside that shared comparison content; packet identity and outputs are deterministic and deeply frozen. | Implemented as a local server-authored packet consumed by both surface candidates; exact committed route/build binding, rendered desktop and 320 CSS px parity, keyboard and access burden, salience, difficulty, independent review, and approval remain open |
