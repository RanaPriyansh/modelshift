...
state is caller-asserted synthetic plan coherence. It does not establish a
matched-substitute artifact, P/Q pack equivalence, approval, or rehearsal
readiness. Participant recruitment, enrollment, observation, and capture
remain blocked, and `UV1-GATE-001`, `UV1-GATE-002`, and `UV1-GATE-003` remain
open.
...
| `UV1-OBS-002` | Use one fixed credible matched substitute rather than a blank or deliberately weak control. | Neutral worksheet carries the same synthetic facts, choices, tasks, timebox, access constraints, and no-effect boundaries without FORGE hierarchy, brand, scoring, or automation. | Plan declaration bound; artifact not authored or approved |
