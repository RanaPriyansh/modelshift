...
export function useForgeSprintStore() {
  ...
  function readRawStore(): string | null {
    ...
    return readRawForgeSprintStore(window.localStorage);
  }

  function clearUnreadableStore(): string | null {
    if (typeof window === "undefined") return "Browser storage is not available.";
    if (!blocked) return "No unreadable sprint data is present.";
    try {
      deleteForgeSprintStore(window.localStorage);
      setState({
        ready: true,
        store: createEmptyForgeSprintStore(),
        issues: [],
      ...
      return "FORGE could not clear the unreadable data. Nothing was changed.";
    }
  }
