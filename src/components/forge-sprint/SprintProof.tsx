...
export function SprintProof({ sprintId }: { sprintId: string }) {
  ...
  const projectComplete = sprint.status === "completed" && completedDays === 7;
  const proofComplete = sprint.proofLab.status === "self_declared" && projectComplete;
  const proofContaminated = sprint.proofLab.status === "contaminated";
      ...
      <main className="forge-proof-page" id="forge-sprint-main" tabIndex={-1}>
        ...
        <section className="forge-proof-boundary">
          ...
          <div>
            <strong>
              {proofContaminated
                ? "Learning attempt retained; independent proof unavailable"
                : "Learner-declared, scoped proof"}
            </strong>
            <p>
              {describeForgeProofLab(sprint.proofLab)}. This page records what the learner says they built
              and the evidence they attached. FORGE has not independently verified identity, authorship,
              mastery, or every external link.
            </p>
          </div>
        </section>
            ...
            <section className="forge-proof-section forge-proof-lab">
              ...
              <div>
                <header className="forge-proof-lab-title">
                  ...
                  <strong>
                    {sprint.proofLab.status === "self_declared"
                      ? "Self-declared"
                      : sprint.proofLab.status === "contaminated"
                        ? "AI-assisted or uncertain"
                        : "Not completed"}
                  </strong>
                </header>
        ...
        <footer className="forge-proof-footer">
          <span>Built with FORGE Project Sprint · {example ? "Example data" : "Stored on this browser"}</span>
