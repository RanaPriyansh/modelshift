...
export function SprintWorkspace({ sprintId }: { sprintId: string }) {
  ...
  function completeDay() {
    ...
    setErrors([]);
    if (result.sprint.status === "completed") {
      setSaveStatus(
        result.sprint.proofLab.status === "contaminated"
          ? "Sprint complete. The project record is ready; independent proof still needs a fresh protected pass."
          : "Sprint complete. Your proof is ready to inspect.",
      );
    } else if (currentDraft.currentDay === 6 && result.sprint.proofLab.status === "contaminated") {
      setSaveStatus("Day complete. This attempt remains visible, but it does not create an independent-proof claim.");
    } else {
      setSaveStatus("Day complete. The next move is ready.");
    }
  }
        ...
        <article className="forge-today-move">
            ...
            <section className="forge-proof-lab-fields" aria-labelledby="proof-lab-heading">
              ...
              <fieldset className="forge-ai-declaration">
                ...
                <label>
                  ...
                  />
                  I used AI or I am unsure. Keep this attempt visible and continue without an independent-proof claim.
                </label>
              </fieldset>
            </section>
          ) : null}
          ...
          {currentDraft.currentDay === 7 ? (
            <section className="forge-delivery-fields" aria-labelledby="delivery-heading">
              ...
              {currentDraft.proofLab.status === "contaminated" ? (
                <p className="forge-proof-contaminated" role="status">
                  The protected pass was declared AI-assisted or uncertain. Finish the project record honestly;
                  FORGE will retain the attempt but make no independent-proof claim.
                </p>
              ) : null}
              <label>
