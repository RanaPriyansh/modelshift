...
export interface ForgeProofLab {
  ...
  status: "not_started" | "self_declared";
}
...
export function validateDayCompletion(
  ...
  if (dayNumber === 6) {
    ...
    if (sprint.proofLab.aiUse !== "learner_declares_no_ai") {
...
export function buildForgeProofSummary(sprint: ForgeSprint): string {
  ...
  return [
    ...
    "Proof Lab: " + (sprint.proofLab.status === "self_declared" ? "self-declared, browser-local" : "not completed"),
...
export function buildForgeProofMarkdown(sprint: ForgeSprint): string {
  ...
  return [
    ...
    sprint.audience,
    "",
    "> 7-day Forge sprint. Completed locally. Not independently verified.",
    "",
    "## Finish line",
    "",
    sprint.finishLine,
    ...
    "## Proof Lab",
    "",
    "Status: " + (sprint.proofLab.status === "self_declared" ? "Self-declared, browser-local" : "Not completed"),
    "",
    ...
    "",
    "---",
    "Built with Forge. The artifact can be inspected; the proof remains scoped.",
  ].join("\n");
}
...
function isProofLab(value: unknown): value is ForgeProofLab {
  ...
  return (
    ...
    (value.status === "not_started" || value.status === "self_declared")
  );
}
