...
async function readBoundedBody(request: Request): Promise<string | null> {
  ...
  if (
    ...
    && declaredLength > MAX_FORGE_PLAN_REQUEST_BYTES
  ) return null;
...
type PlanRouteDependencies = {
  readonly plan: (
    input: ForgePlanRequest,
  ) => ReturnType<typeof planForgeLearning>;
};
...
export function createForgePlanPost(
  dependencies: PlanRouteDependencies = DEFAULT_DEPENDENCIES,
) {
  return async function forgePlanPost(request: Request) {
    const contentType =
    ...
    const parsed = forgePlanApiRequestSchema.safeParse(body);
